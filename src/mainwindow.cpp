    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(ui->toolButton_draw, SIGNAL(clicked(bool)),this, SLOT(slotBtnClicked(bool)));
    ...
    connect(mCaptureTaskManager,&CaptureTaskManager::sig_PressEsc,[=](){
        ui->toolButton_draw->setChecked(false);
    });
...
void MainWindow::slotTimerTimeOut()
{
    if (QObject::sender() == mTimer_GetTime)
    {
        ///设置时间
        {
            ...
            ////> 检测录制时长
            if(currentTime >= AppConfig::gRecordDuration * 60 * 60)
            {
                ///> 停止录制
                stopRecord();
            }
...
void MainWindow::slotBtnClicked(bool isChecked)
{
    ...
    else if (QObject::sender() == ui->toolButton_record_start) ///> 开始录制
    {
        startRecord();
    }
    ...
    else if(QObject::sender() == ui->toolButton_draw)
    {
        if(ui->toolButton_draw->isChecked())
        {
            mCaptureTaskManager->startDraw();
        }
        else
        {
            mCaptureTaskManager->stopDraw();
        }
