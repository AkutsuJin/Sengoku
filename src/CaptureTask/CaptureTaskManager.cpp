...
//    ui->verticalLayout_videoPlayer->addWidget(mPlayVideoTaskWidget);
    ...
    ui->checkBox_picture->hide();
    ui->pushButton_picture->hide();
    ui->label_5->hide();
    ui->horizontalSlider_Size_Picture->hide();
    ui->label_8->hide();
    ...
    m_draw = new DrawWidget(mSelectAreaWidget);
    m_draw->hide();

    connect(mSelectAreaWidget,&SelectAreaWidget::sig_PressEsc,[=](){
        m_draw->hide();
        emit sig_PressEsc();
    });


    m_tool = new DrawTool(0,0,250,250,m_draw);
    m_tool->show();

    connect(m_tool,SIGNAL(sigTool(int,bool)),this,SLOT(onTool(int,bool)));
    connect(m_tool,SIGNAL(sigSelectColor(QColor)),this,SLOT(onColorCom(QColor)));
    connect(m_tool,SIGNAL(sigSelectShape(int)),this,SLOT(onCom(int)));
...
//qDebug()<<__FUNCTION__<<"AppConfig::gVideoQuality="<<AppConfig::gVideoQuality;
    ...
    m_GIFManager = new GIFManager(this);
...
{
    if(AppConfig::gRecordMode == 1)
    {
        return m_GIFManager->getVideoFileCurrentTime();
    }
    else
    {
...
void CaptureTaskManager::onCom(int val)
{
    m_draw->setShape(val);

}

void CaptureTaskManager::onColorCom(QColor val)
{
    m_draw->setColor(val);
}

void CaptureTaskManager::onTool(int value,bool flag)
{
    switch (value) {
    case 0:
    {
...
//    mMediaManager->setQuantity(AppConfig::gVideoQuality);
    ...
    if(AppConfig::gRecordMode == 1) ///> GIF录制
    {
        QRect capRect = mCaptureWindowWidget->getCaptureRect();

        ///> 路径拼接
        QString filePath = QString("%1/%2.gif").arg(AppConfig::gVideoDirPath).arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hhmmss"));
        m_GIFManager->addCaptureWindowTask(0,filePath,capRect);
        m_GIFManager->startRecord();
...
void CaptureTaskManager::startDraw()
{
    QRect capRect = mCaptureWindowWidget->getCaptureRect();
    QScreen *screen = QApplication::primaryScreen();
    //    QPixmap pix = screen->grabWindow(0, x() + rectGif.x(), y() + rectGif.y(), rectGif.width(), rectGif.height());
    QPixmap pix = screen->grabWindow(0, capRect.x(), capRect.y(), capRect.width(), capRect.height());

    m_draw->setGeometry(capRect);
    m_draw->setCanvas(pix);
    m_draw->show();
}

void CaptureTaskManager::stopDraw()
{
    m_draw->hide();
}
...
//            qDebug()<<__FUNCTION__<<IsWindowVisible(hWnd)<<IsIconic(hWnd);
    ...
    else if (QObject::sender() == mTimer_GetTime)
    {
        ///获取录屏时间
        {
            ...
            currentTime = getVideoFileCurrentTime();
            currentTime /= 1000;
