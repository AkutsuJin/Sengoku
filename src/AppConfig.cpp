...
QImage AppConfig::gLogoImage;
int AppConfig::gLogoDir = 0; ///> -1:不添加水印 0:左上 1:左下 2:中心 3:右上 4:右下
...
int AppConfig::gRecordMode = 0; //录制模式  0：本地录制,1:GIF录制,2:推流
QString AppConfig::gRtmpUrl = "rtmp://127.0.0.1:1935/live/1";  //推流地址
double AppConfig::gRecordDuration = 1.0; ///> 录制时长,单位小时
...
//    QString dataPath = QCoreApplication::applicationDirPath();
    ...
    gLogoImage = QImage(QString("%1/logo.png").arg(AppDataPath_Data));
...
//                    if (h <= 0) y=288;
                    ...
                    AppConfig::gRecordDuration = recorderObject.value("recordduration").toDouble();
                    AppConfig::gRecordMode = recorderObject.value("mode").toInt(); //> 录制模式
                    AppConfig::gLogoDir = recorderObject.value("logo").toInt();
                }
                ...
                ///> 读取推流信息
                QJsonValue rtmpValue = object.value("rtmp");
                if(rtmpValue.isObject())
                {
                    QJsonObject rtmpObject = rtmpValue.toObject();
                    AppConfig::gRtmpUrl = rtmpObject.value("url").toString();
...
void AppConfig::saveConfigInfoToFile()
{
    ...
    if (file.open(QIODevice::WriteOnly))
    {
        ...
        ///记录录屏相关配置
        {
            ...
            object.insert("recordduration", AppConfig::gRecordDuration);
            object.insert("mode", AppConfig::gRecordMode);
            object.insert("logo",AppConfig::gLogoDir);
...
//                videoSizeObject.insert("y", AppConfig::gVideoFileSize.y());
        ...
        {
            QJsonObject object;
            object.insert("url", AppConfig::gRtmpUrl);
            dataObject.insert("rtmp", object);
        }
