    ...
    ui(new Ui::DeviceSettingDialog)
{
    ...
    ui->doubleSpinBox->setValue(AppConfig::gRecordDuration);

    QPixmap pixmap(QString("%1/logo.png").arg(AppConfig::AppDataPath_Data));
    if(pixmap.isNull() == false)
    {
        QSize size = getMaxRatioRect(ui->label_image->size(),pixmap.size());
        pixmap = pixmap.scaled(size,Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
        ui->label_image->setPixmap(pixmap);
    }


    switch (AppConfig::gLogoDir) {
    case 0:
...
void DeviceSettingDialog::slotBtnClick(bool isChked)
{
    ...
    else if (QObject::sender() == ui->pushButton_confirm) ///> 确定
    {

        if(ui->radioButton->isChecked())
        {
            AppConfig::gRecordMode = 0;  ///> 本地录制
        }
        else if(ui->radioButton_2->isChecked())
        {
            AppConfig::gRecordMode = 1; ///> GIF录制
        }
        else if(ui->radioButton_3->isChecked())
        {
            AppConfig::gRecordMode = 2; ///>推流
        }


        if(ui->radioButton_4->isChecked())
        {
            AppConfig::gLogoDir = 0;
        }
        else if(ui->radioButton_5->isChecked())
        {
            AppConfig::gLogoDir = 1;
        }
        else if(ui->radioButton_6->isChecked())
        {
            AppConfig::gLogoDir = 2;
        }
        else if(ui->radioButton_7->isChecked())
        {
            AppConfig::gLogoDir = 3;
        }
        else if(ui->radioButton_8->isChecked())
        {
            AppConfig::gLogoDir = 4;
        }
        ...
        AppConfig::gRtmpUrl = ui->lineEdit->text();
        AppConfig::gRecordDuration = ui->doubleSpinBox->value();
...
QSize DeviceSettingDialog::getMaxRatioRect(const QSize &container, const QSize &inner)
{
    ...
    double containerScaled = container.width() * 1.0 / container.height();
    double innerScaled = inner.width() * 1.0 / inner.height();

    if (innerScaled < containerScaled) {
        int height = container.height();
        destSize.setHeight(height);
        int width = height *innerScaled;
        destSize.setWidth(width);
    }
    else if (innerScaled == containerScaled) {
        destSize.setWidth(container.width());
        destSize.setHeight(container.height());
    }
    else {
        int width = container.width();
        destSize.setWidth(width);
        int height = width / innerScaled;
        destSize.setHeight(height);
    }
    return destSize;
}

void DeviceSettingDialog::on_pushButton_clicked()
{
    ...
    if(fileName != "")
    {
        ...
        if(pixmap.isNull() == false)
        {
            AppConfig::gLogoImage = QImage(fileName);

            QFile::remove(QString("%1/logo.png").arg(AppConfig::AppDataPath_Data));
            pixmap.save(QString("%1/logo.png").arg(AppConfig::AppDataPath_Data));
            QSize size = getMaxRatioRect(ui->label_image->size(),pixmap.size());
            pixmap = pixmap.scaled(size,Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
            ui->label_image->setPixmap(pixmap);
        }
