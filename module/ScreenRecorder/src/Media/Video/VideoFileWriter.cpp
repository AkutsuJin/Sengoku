...
//                .arg(fileInfo.suffix());
    ...
    if(AppConfig::gRecordMode == 2) ////> 推流
    {
        filename = AppConfig::gRtmpUrl.toStdString().c_str();
        avformat_alloc_output_context2(&oc, 0, "flv", filename); ////> 推流模式
        if (!oc)
        {
            printf("Could not push stream;\n");
            continue;
        }
    }
    else if(AppConfig::gRecordMode == 0) ///> 本地录制
    {
        /* allocate the output media context */
        avformat_alloc_output_context2(&oc, NULL, NULL, filename); ////> 写入文件模式
        if (!oc)
        {
